...
def load_model(num_classes=2, model_name="mbv3", border=False, device=torch.device("cpu")):
    if model_name == "mbv3":
        model = deeplabv3_mobilenet_v3_large(num_classes=num_classes, aux_loss=True)
        if border:
            checkpoint_path = os.path.join(os.getcwd(), "model_trained_iou_mix_2C_aux.pth")
        else:
...
def scan(image_true=None, trained_model=None, image_size=384, BUFFER=10):
    ...
    final = cv2.warpPerspective(image_true, M, (destination_corners[2][0], destination_corners[2][1]),
